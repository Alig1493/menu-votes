...
class TestMenu:
    ...
    def test_menu_create_with_multipart_image(self, auth_client, image_file, temp_media, restaurant):
        assert len(list(temp_media.iterdir())) == 0
        ...
        assert len(list(temp_media.iterdir())) == 1

    def test_menu_create_with_base64_image(self, auth_client, base64_image, temp_media, restaurant):
        assert len(list(temp_media.iterdir())) == 0
        ...
        response = auth_client.post(self.url, data=data, format="json")
    ...
    def test_menu_filter(self, auth_client):
        # this is due to the fact that datetimestamps with auto_now and auto_now_add doesn't allow us
        # to create objects with a datetime other than the past. SInce queryset update hits the db directly
        # we can override the field values for the purpose of our testing
